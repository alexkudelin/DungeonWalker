...
func set_current_selection(_menu_level, _first_level_item, _selected_algo):
	seed_marker.text = ""
...
func _handle_first_level_menu():
	if Input.is_action_just_pressed("ui_down") and first_level_item < 3:
		first_level_item += 1
		set_current_selection(menu_level, first_level_item, selected_algo)
...
func _handle_second_level_menu():
	if Input.is_action_just_pressed("ui_down") and selected_algo < 2:
		if first_level_item == 1:
			selected_algo += 1
			set_current_selection(menu_level, first_level_item, selected_algo)
	elif Input.is_action_just_pressed("ui_up") and selected_algo > 0:
		if first_level_item == 1:
			selected_algo -= 1
			set_current_selection(menu_level, first_level_item, selected_algo)
	elif Input.is_action_just_pressed("ui_accept"):
